...
public partial class Vista_Tools_CargaGenericaImpuesto : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!iSessionE)
        {
            ...
            try
            {
                ...
                string[] datos = { "Inicio", "ToolsImpuesto", "Carga másiva" };
                string[] url = { "", "", "" };
    ...
    public static void fn_CargaMasiva(string sFileName)
    {
        ...
        ToolsImpuesto objToolsImpuesto = new ToolsImpuesto();
        objToolsImpuesto.sFileName = sFileName;
        Thread objThread = new Thread(new ParameterizedThreadStart(objToolsImpuesto.fn_ProcesaArchivo));
        objThread.Start(new object[] { HttpContext.Current.Session, HttpContext.Current });
    }

    [WebMethod(EnableSession = true)]
    public static ToolsImpuesto fn_LeerProgreso()
    {
        ToolsImpuesto objToolsImpuesto = new ToolsImpuesto();
        objToolsImpuesto.fProgreso = (float)HttpContext.Current.Session["fProgresoCarga"];

        if ((bool)HttpContext.Current.Session["fCompletado"])
        {
            objToolsImpuesto.fProgreso = 80f;
            objToolsImpuesto.lstErrores = (List<string>)HttpContext.Current.Session["lstErrores"];
            objToolsImpuesto.fCompletado = true;
        ...
        {
            objToolsImpuesto.fCompletado = false;
        }
        return objToolsImpuesto;
    }
}